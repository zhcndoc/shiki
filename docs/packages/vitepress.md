...
你可以为生成的类型启用内联缓存。`@twoslash-cache: ...` 会在开发和构建时自动插入到你的代码块中。
...
要在你的 [`.vitepress/config.ts`](https://vitepress.dev/reference/site-config) 中启用内联缓存，请使用由 `createTwoslashWithInlineCache` 创建的配置包装器。
...
const withTwoslashInlineCache = createTwoslashWithInlineCache({ // [!code hl]
  // ... transformerTwoslash 的配置 // [!code hl]
}) // [!code hl]
...
export default withTwoslashInlineCache( // [!code hl]
  defineConfig({
    markdown: {
      codeTransformers: [
        // 配置已移至 `createTwoslashWithInlineCache()` // [!code error]
...
如果想强制重新生成内联缓存并忽略已有缓存，可以在运行 `vitepress` CLI 时设置环境变量 `TWOSLASH_INLINE_CACHE_IGNORE`。
...
如果想移除所有内联缓存，可以在运行 `vitepress` CLI 时设置环境变量 `TWOSLASH_INLINE_CACHE_REMOVE`。
